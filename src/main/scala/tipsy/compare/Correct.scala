...
object Correct {
  ...
  def apply(t1: ParseTree, t2: ParseTree): Either[CorrError, EditRet] = {
    for {
    ...
    } yield (LeastEdit.compareTwoTrees(main1, main2))
  }
  ...
  private def getFxnByName(tree: List[ParseTree], n: String) = {
    tree collect {
      case x @ FxnDefinition(n, _, Some(body))
          if n.name.toString == "main" => body
    } headOption match {
