 ...
 */
  ...
  val powersOfE: Array[Double] = PowersOfN(exp(1.0 / 3.0), 300)
  ...
  def editDistRecurExpr(m: Int, n: Int): Unit = {
    ...
    for (i <- 1 to m) {
      ...
      editDistExpr(i)(0) = (a + powersOfE(b / 5 + 1), (b + 1, c))
    }
    for (j <- 1 to n) {
      val (a, (b, c)) = editDistExpr(0)(j - 1)
      editDistExpr(0)(j) = (a + powersOfE(b / 5 + 1), (b + 1, c))
    }
    for (i <- 1 to m) {
      for (j <- 1 to n) {
        if (string1(i - 1) == string2(j - 1)) {
          ...
          val penalty: Array[Double] = Array(-1.0, a(1) + powersOfE(b(1) / 5 + 1), a(2) + powersOfE(b(2) / 5 + 1), a(3) + powersOfE(c(3) / 5 + 1))
          val eps = 1e-6
          var ind = 1
          ...
          if (ind == 1) {
            ...
            editDistExpr(i)(j) = (penalty(3), (b(3) + 1, c(3) + 1))
          }
        }
      }
    }
  ...
  def editDistRecur(m: Int, n: Int): Unit= {
    editDist(0)(0) = (EditRet(List(), 0), (0, 0))
    for (i <- 1 to m) {
      val (a, (b, c)) = editDist(i - 1)(0)
      editDist(i)(0) = (a.correct(Diff(DEL_d, None, Some(cfenum1(i - 1))), a.dist + powersOfE(b / 7 + 1)), (b + 1, c))
    }
    for (j <- 1 to n) {
      val (a, (b, c)) = editDist(0)(j - 1)
      editDist(0)(j) = (a.correct(Diff(ADD_d, Some(cfenum2(j - 1)), None), a.dist + powersOfE(b / 7 + 1)), (b + 1, c))
    }
    for (i <- 1 to m) {
      for (j <- 1 to n) {
        if (cfenum1(i - 1) == cfenum2(j - 1)) {
          ...
          val penalty: Array[Double] = Array(-1.0, a(1).dist + powersOfE(b(1) / 7 + 1), a(2).dist + powersOfE(b(2) / 7 + 1), a(3).dist + powersOfE(c(3) / 7 + 1))
          val eps = 1e-6
          var ind = 1
...
//              println(l1, l2)
          ...
          penalty(3) = penalty(3) min (a(3).dist + d)
